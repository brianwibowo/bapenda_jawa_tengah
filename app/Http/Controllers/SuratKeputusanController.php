...
class SuratKeputusanController extends Controller
{
    ...
    public function generateSkJR(Request $request, Pengajuan $pengajuan)
    {
        $request->validate([
            ...
            'tanggal_surat_permohonan' => 'required|string',
            'nomor_surat_regident' => 'required|string',
            'tanggal_surat_regident' => 'required|string',
            'nomor_surat_bapenda' => 'nullable|string',
            'tanggal_surat_bapenda' => 'nullable|string',
        ...
        foreach ($kendaraan as $k) {
            $dataPdf = [
                ...
                'nomor_surat_bapenda' => strtoupper($request->nomor_surat_bapenda ?? '_________'),
                'tanggal_surat_bapenda' => strtoupper($request->tanggal_surat_bapenda ?? '_________'),
                ...
                'jabatan_penandatangan' => 'KEPALA KANTOR WILAYAH UTAMA PT JASA RAHARJA JAWA TENGAH',
                ...
                'data' => (object)[
                    ...
                    'merk_type' => ($k->merk_kendaraan ?? '-') . '/' . ($k->tipe_kendaraan ?? '-'),
                    'no_rangka_mesin' => ($k->nomor_rangka ?? '-') . '/' . ($k->nomor_mesin ?? '-'),
            ...
            // Generate PDF
            $pdf = Pdf::loadView('pdf.sk_jasa_raharja_pembebasan', $dataPdf)->setPaper('a4', 'portrait');
        ...
        foreach ($kendaraan as $k) {
            $dataPdf = [
                ...
                'data' => (object)[
                    'nama' => optional($k->pemilik)->nama_pemilik ?? '-',
                    'alamat' => optional($k->pemilik)->alamat_pemilik ?? '-',
                    'nik' => optional($k->pemilik)->nik_pemilik ?? '-',
                    'no_tlp' => optional($k->pemilik)->telp_pemilik ?? '-',
                    'email' => optional($k->pemilik)->email_pemilik ?? '-',
                    'nrkb' => $k->nrkb ?? '-',
                    'merek' => $k->merk_kendaraan ?? '-',
                    'tipe' => $k->tipe_kendaraan ?? '-',
                    'jenis' => $k->jenis_kendaraan ?? '-',
                    'model' => $k->model_kendaraan ?? '-',
                    'tahun' => $k->tahun_pembuatan ?? '-',
                    'isi_silinder' => $k->isi_silinder ?? '-',
                    'no_rangka' => $k->nomor_rangka ?? '-',
                    'no_mesin' => $k->nomor_mesin ?? '-',
                    'warna_kendaraan' => $k->warna_kendaraan ?? '-',
                    'bahan_bakar' => $k->jenis_bahan_bakar ?? '-',
                    'warna_tnkb' => $k->warna_tnkb ?? '-',
                    'no_bpkb' => $k->nomor_bpkb ?? '-',
                ]
            ];
            ...
            $pdf = Pdf::loadView('pdf.sk_polda', $dataPdf)->setPaper('a4', 'portrait');
            if ($request->has('preview')) {
                $previewDir = 'sk/preview';
