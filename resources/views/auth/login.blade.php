    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mb-4">
            ...
            <div class="input-group shadow-sm">
                ...
                <input id="email" type="email" class="form-control bg-white border-start-0 ps-0 @error('email') is-invalid @enderror" name="email" :value="old('email')" required autofocus autocomplete="username" placeholder="nama@bapenda.go.id">
            </div>
            @error('email')
        ...
        <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center mb-1">
                ...
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-decoration-none small text-primary fw-semibold hover-primary">
                        Lupa Password?
                    </a>
                @endif
            </div>
        ...
        <div>
            <button type="submit" class="btn btn-primary shadow-sm d-block w-100 py-2 fw-bold text-white" style="background: linear-gradient(to right, #0D8ABC 0%, #00509E 100%); border: none; border-radius: 8px;">
